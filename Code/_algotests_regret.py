...
## newly add
def bayes_ucb_gaussian_squared(scenario, n_sims, n_horizon, c=2, real_sd = 0.1):
    dd = r'C:/Users/lpy20/Desktop/UCLA/ML in Chem/optimization data logs/optimization data logs/synthetic data simulation logs/normal arm'
    #fp = f'{dd}/scenario{scenario}/BayesUCBGaussian/BayesUCBGaussian_squared/NormalArm_c={c}' #implementation 1
    fp = f'{dd}/scenario{scenario}/BayesUCBGaussian/BayesUCBGaussian_PPF_squared/NormalArm_c={c}' #implementation 2
    output_dir = make_dir(fp)

    means = means_from_scenario(scenario)
    n_arms = len(means)
    sds = [real_sd for i in range(n_arms)]

    def build(x, y):
        return NormalArm(x, y)
    arms = list(map(build, means, sds))

    for assume_sd in [0.1, 0.25, 0.5, 0.75, 1, 1.5]:
        #algo = BayesUCBGaussianSquaredModifySd(n_arms, assumed_sd=assume_sd, c=c) #implementation 1
        algo = BayesUCBGaussianPPFSquaredModifySd(n_arms, assumed_sd=assume_sd, c=c) #implementation 2
        algo.reset(n_arms)
        results = test_algorithm_regret(algo, arms, n_sims, n_horizon)
        filename = f'bayes_ucb_gaussian_squared_realsd_{real_sd}_assumed_sd_{assume_sd}.csv'
        results.to_csv(output_dir / filename)
...
#for real_sd in real_sd_values:
   # bayes_ucb_gaussian_squared(scenario=1, n_sims=1000, n_horizon=250, c=2, real_sd=real_sd)

c_values = [1, 1.5, 2, 2.5, 3]
real_sd_values = [0.1, 0.25, 0.5, 0.75, 1, 1.5]
#for c in c_values:
 #   for real_sd in real_sd_values:
  #      bayes_ucb_gaussian_squared(scenario=2, n_sims=1000, n_horizon=250, c=c, real_sd=real_sd)

#bayes_ucb_gaussian_squared(scenario=1, n_sims=1000, n_horizon=250, c=2, real_sd=2)
...
# added
# use to test ts gaussian square with different assumed sd at different real sd
def ts_gaussian_squared(scenario, n_sims, n_horizon, real_sd=0.25):
    dd = r'C:/Users/lpy20/Desktop/UCLA/ML in Chem/optimization data logs/optimization data logs/synthetic data simulation logs/normal arm'
    fp = f'{dd}/scenario{scenario}/TS/TS_squared'
    output_dir = make_dir(fp)
    ...
    for assume_sd in [0.1, 0.25, 0.5, 0.75, 1, 1.5]:
        # Initialize Thompson Sampling algorithm with fixed variance
        algo = ThompsonSamplingGaussianFixedVarSquared(n_arms, assumed_sd=assume_sd)
        algo.reset(n_arms)
        
        # Test algorithm and save results
        results = test_algorithm_regret(algo, arms, n_sims, n_horizon)
        filename = f'ts_gaussian_squared_realsd_{real_sd}_assumed_sd_{assume_sd}.csv'
        results.to_csv(output_dir / filename)
...
real_sd_values = [0.1, 0.25, 0.5, 0.75, 1, 1.5]
#for real_sd in real_sd_values:
 #   ts_gaussian_squared(scenario=2, n_sims=1000, n_horizon=250, real_sd=real_sd)
...
#add
def TS_gaussian(scenario, n_sims, n_horizon, real_sd):
    dd = r'C:/Users/lpy20/Desktop/UCLA/ML in Chem/optimization data logs/optimization data logs/synthetic data simulation logs/normal arm'
    fp = f'{dd}/scenario{scenario}/TS/TS'
    output_dir = make_dir(fp)
    ...
    for assume_sd in [0.1, 0.25, 0.5, 0.75, 1, 1.5]:
        # Initialize Thompson Sampling algorithm with fixed variance
        algo = ThompsonSamplingGaussianFixedVar(n_arms, assumed_sd=assume_sd)
        algo.reset(n_arms)
        
        # Test algorithm and save results
        results = test_algorithm_regret(algo, arms, n_sims, n_horizon)
        filename = f'ts_gaussian_realsd_{real_sd}_assumed_sd_{assume_sd}.csv'
        results.to_csv(output_dir / filename)

    return None
real_sd_values = [0.1, 0.25, 0.5, 0.75, 1, 1.5]
for real_sd in real_sd_values:
    TS_gaussian(scenario=2, n_sims=1000, n_horizon=250, real_sd=real_sd)
for real_sd in real_sd_values:
    TS_gaussian(scenario=1, n_sims=1000, n_horizon=250, real_sd=real_sd)

#add
def bayes_ucb_gaussian(scenario, n_sims, n_horizon, c=2, real_sd = 0.1):
    dd = r'C:/Users/lpy20/Desktop/UCLA/ML in Chem/optimization data logs/optimization data logs/synthetic data simulation logs/normal arm'
    fp = f'{dd}/scenario{scenario}/BayesUCBGaussian/BayesUCBGaussian'
    output_dir = make_dir(fp)
    ...
    for assume_sd in [0.1, 0.25, 0.5, 0.75, 1, 1.5]:
        algo = BayesUCBGaussian(n_arms, assumed_sd=assume_sd, c=c)
        algo.reset(n_arms)
        results = test_algorithm_regret(algo, arms, n_sims, n_horizon)
        filename = f'bayes_ucb_gaussian_realsd_{real_sd}_assumed_sd_{assume_sd}.csv'
        results.to_csv(output_dir / filename)
...
real_sd_values = [0.1, 0.25, 0.5, 0.75, 1, 1.5]
for real_sd in real_sd_values:
    bayes_ucb_gaussian(scenario=2, n_sims=1000, n_horizon=250, real_sd=real_sd)
for real_sd in real_sd_values:
    bayes_ucb_gaussian(scenario=1, n_sims=1000, n_horizon=250, real_sd=real_sd)
